...
namespace IECSC.Grouping.Action
{
    class OpcAction
    {
        ...
        public bool ConnectOpc(ref string errMsg)
        {
            try
            {
                var result = opcClient.ConnectOpcServer(McConfig.Instance.OpcServerIp, McConfig.Instance.OpcServerName, ref errMsg);
                if (result)
                {
        ...
        public bool AddOpcGroup(ref string errMsg)
        {
            try
            {
                return opcClient.AddOpcGroup(McConfig.Instance.OpcGroupName, ref errMsg);
        ...
        public bool WriteTaskCmd(Loc loc, ref string errMsg)
        {
            try
            {
                ...
                foreach (var item in BizHandle.Instance.writeItems)
                {
                    ...
                    {
                        case "Write.SerialNo":
                            keyValues.Add(new KeyValuePair<string, object>(item.Key, loc.taskCmd.SerialNo));
                            break;
                        case "Write.PalletNo":
