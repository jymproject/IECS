...
using IECSC.Grouping.CustomControl;
using System;
...
namespace IECSC.Grouping
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void ShowLocStatus(string locNo)
        {
            ...
            locControlDic[locNo].RealWeight = loc.plcStatus.RealWeight??"0".ToString();
        ...
        private void ShowTaskCmd()
        {
            ...
            if (string.IsNullOrEmpty(LimitLocNo))
            {
                ...
                {
                    var list = DbAction.Instance.GetTaskCmd(loc.LocNo);
        ...
        private void btnRefSend_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var selectItem = (DownLoadInfo)dgv.SelectedItem;
                var cmdId = selectItem.Objid;
                var locNo = selectItem.SlocNo;
                var whNo = selectItem.WhNo;
                if (cmdId == 0)
                {
                ...
                {
                    var result = DbAction.Instance.UpdateCmdStep(cmdId, whNo, "00");
                    if (result)
                    {
        ...
        private void btnFinish_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var selectItem = (DownLoadInfo)dgv.SelectedItem;
                var taskNo = selectItem.SerialNo;
                var locNo = selectItem.SlocNo;
                ...
                {
                    var errMsg = string.Empty;
                    var requestId = DbAction.Instance.GetCmdFinishObjid(whNo);
                    if(requestId <= 0)
                    {
                        MessageBox.Show($"获取指令结束请求OBJID失败", "提示", MessageBoxButton.YesNo, MessageBoxImage.Error);
                    }

                    var result = DbAction.Instance.RequestFinishCmd(whNo, requestId, taskNo, ElocNo, 2, ref errMsg);
                    if (result)
                    {
                        BizHandle.Instance.locDic[locNo].bizStatus = BizStatus.None;
                        ShowTaskCmd();
        ...
        private void btnDel_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    var errMsg = string.Empty;
                    var result = DbAction.Instance.DeleteTaskCmd(whNo, taskNo, ref errMsg);
                    if (result)
                    {
                        ShowTaskCmd();
