    ...
    public partial class DbAction
    {
        ...
        public DataTable GetTaskCmd(Loc loc, string cmdStep)
        {
            try
            {
                ...
                //如果站台为机械手码垛工位，则判断是否有到达此站台的指令
                if (loc.TaskType == "StackLocBiz")
                {
                    sb.Append(" WHERE T.ELOC_NO = @LOCNO");
                }
                else
                {
                    sb.Append(" WHERE T.SLOC_NO = @LOCNO");
            ...
            {
                log.Error($"[异常]执行GetTaskCmd({loc.LocNo}, {cmdStep})获取指令信息失败:{ex.ToString()}");
                return null;
            }
        }
        /// <summary>
        ...
        /// 获取任务与指令请求参数表objid
        /// </summary>
        public int GetObjidForRequestTaskAndCmd()
        {
            try
            {
                return Db.Connection.ExecuteScalar<int>("SELECT NEXT VALUE FOR SEQ_TPROC_0100_TASK_CMD_CREATE");
            }
            catch (Exception ex)
            {
                log.Error($"[异常]执行GetObjidForRequestTask()获取请求生成任务参数表主键ID失败:{ex.Message}");
        ...
        /// 请求生成任务
        /// </summary>
        public int RequestTaskAndCmd(long objid, string elocNo, string orderTypeNo, ref string errMsg)
        {
            try
            {
                var dt = Db.Connection.QueryTable($"SELECT * FROM TPROC_0100_TASK_CMD_CREATE T WHERE T.OBJID = {objid}");
                if (dt == null || dt.Rows.Count == 0)
                {
                    var sb = new StringBuilder();
                    sb.Append(" INSERT INTO TPROC_0100_TASK_CMD_CREATE");
                    sb.Append(" (OBJID, ORDER_TYPE_NO, ELOC_NO)");
                    sb.Append(" VALUES ");
                    sb.Append(" (@OBJID, @ORDERTYPENO ,@ELOCNO)");
                    var param = new DynamicParameters();
                    param.Add("OBJID", objid);
                ...
                else
                {
                    ...
                    {
                        string strSql = $"update TPROC_0100_TASK_CMD_CREATE set ORDER_TYPE_NO={orderTypeNo},ELOC_NO={elocNo} where OBJID={objid}";
                ...
                dp.Add("O_ERR_CODE", 0, DbType.Int32, ParameterDirection.Output);
                dp.Add("O_ERR_DESC", 0, DbType.String, ParameterDirection.Output, size: 80);
                Db.Connection.Execute("PROC_0100_TASK_CMD_CREATE", param: dp, commandType: CommandType.StoredProcedure);
                errMsg = dp.Get<string>("O_ERR_DESC");
                return dp.Get<int>("O_TASK_NO");
            }
            catch (Exception ex)
            {
                log.Error($"[异常]执行RequestTaskAndCmd({objid},{elocNo},{orderTypeNo},ref string errMsg)请求生成任务与指令失败:{ex.ToString()}");
                errMsg = ex.Message;
                return -1;
            }
        }
        ...
        /// 请求生成指令
        /// </summary>
        //public int RequestCmdForStackloc(int objid, string elocNo, int taskNo, ref string errMsg)
        //{
        //    try
        //    {
        //        //获取是否已经插入数据
        ...
        //            var sb = new StringBuilder();
        //            sb.Append(" INSERT INTO TPROC_0200_CMD_REQUEST");
        //            sb.Append(" (OBJID, TASK_NO, ARRIVAL_LOC_NO)");
        //            sb.Append(" VALUES");
        //            sb.Append(" (@OBJID, @TASKNO, @LOCNO)");
        //            var param = new DynamicParameters();
        //            param.Add("OBJID", objid);
        //            param.Add("TASKNO", taskNo);
        //            param.Add("LOCNO", elocNo);
        //            Db.Connection.Execute(sb.ToString(), param);
        //        }
        //        else
        //        {
        //            string strSql = $"update TPROC_0200_CMD_REQUEST set TASK_NO={taskNo},ARRIVAL_LOC_NO={elocNo} where OBJID={objid}";
        //            Db.Connection.Execute(strSql);
        ...
        //    catch (Exception ex)
        //    {
        //        log.Error($"[异常]执行RequestCmd({objid},{elocNo},{taskNo},ref string errMsg)请求生成指令失败:{ex.ToString()}");
        //        errMsg = ex.Message;
        //        return -1;
        //    }
        //}
